...
function App() {
  ...
  const addStudent = () => {
    if (studentName && studentAmount) {
      setStudents([...students, { year_name: studentName, num_students: studentAmount }]);
      setStudentName("");
      setStudentAmount("");
    }
  };
  ...
  const addActivity = () => {
    if (activityTeacher && activityStudent && subject) {
      setActivities([...activities, { teach_name: activityTeacher, subj_name: subject, year_name: activityStudent }]);
      setActivityTeacher("");
      setActivityStudent("");
      setSubject("");
    }
          ...
          <div className="tab_content">
            ...
            <div>
              <form>
                ...
                <button type="button" onClick={addTeacher}>Add</button>
              </form>
              <ul>
            ...
            <div>
              <form>
                ...
                <input type="number" value={studentAmount} onChange={(e) => setStudentAmount(e.target.value)} required />
            ...
            <div>
              <form>
                ...
                <input type="text" value={buildingInput} onChange={(e) => setBuildingInput(e.target.value)} required />
                <button type="button" onClick={addBuilding}>Add</button>
              </form>
              <ul>
              ...
              <form>
                ...
                <input type="number" value={roomCapacity} onChange={(e) => setRoomCapacity(e.target.value)} required />
            ...
            <div>
              <form>
                <label>Teacher:</label>
                <select value={activityTeacher} onChange={(e) => setActivityTeacher(e.target.value)}>
                  {teachers.map((teacher, index) => (
                    <option key={index} value={teacher.name}>{teacher.name}</option>
                  ))}
                </select>
                <label>Student:</label>
                <select value={activityStudent} onChange={(e) => setActivityStudent(e.target.value)}>
                  {students.map((student, index) => (
                    <option key={index} value={student.year_name}>{student.year_name}</option>
                  ))}
                </select>
                <label>Subject:</label>
                ...
                <button type="button" onClick={addActivity}>Save Activity</button>
              </form>
              <ul>
                {activities.map((activity, index) => (
                  <li key={index}>
                    {activity.teach_name} - {activity.subj_name} - {activity.year_name}
                    <button>Edit</button>
                    <button>Delete</button>
                  </li>
                ))}
